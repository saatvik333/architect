...
class TestSpinDetector:
    ...
    def test_no_spin_on_first_retry(self, spin_detector: SpinDetector) -> None:
        """A single retry should not trigger spin detection."""
        result = spin_detector.record_retry(
    ...
    def test_spin_after_max_retries(self, config: EconomicGovernorConfig) -> None:
        ...
        for _i in range(config.spin_max_retries):
            result = detector.record_retry(agent_id=agent, task_id=task, has_diff=False, tokens=500)
    ...
    def test_diff_resets_counter(self, spin_detector: SpinDetector) -> None:
        """A successful diff should reset the spin counter."""
        agent = AgentId("agent-reset")
        task = TaskId("task-reset")

        # Two retries without diff.
        spin_detector.record_retry(agent_id=agent, task_id=task, has_diff=False, tokens=500)
        spin_detector.record_retry(agent_id=agent, task_id=task, has_diff=False, tokens=500)

        # Diff produced — should reset.
        result = spin_detector.record_retry(agent_id=agent, task_id=task, has_diff=True, tokens=300)
        assert result.is_spinning is False
        assert result.retry_count == 0
        assert result.tokens_since_last_diff == 0

    def test_independent_tracking_per_task(self, spin_detector: SpinDetector) -> None:
        """Spin tracking should be independent per (agent, task) pair."""
        agent = AgentId("agent-multi")
        task_a = TaskId("task-a")
        task_b = TaskId("task-b")

        spin_detector.record_retry(agent_id=agent, task_id=task_a, has_diff=False, tokens=100)
        spin_detector.record_retry(agent_id=agent, task_id=task_a, has_diff=False, tokens=100)

        result_b = spin_detector.record_retry(
    ...
    def test_reset_clears_all_tasks_for_agent(self, spin_detector: SpinDetector) -> None:
        """reset() should clear all spin state for the given agent."""
        agent = AgentId("agent-clear")
        task = TaskId("task-clear")

        spin_detector.record_retry(agent_id=agent, task_id=task, has_diff=False, tokens=100)
        spin_detector.record_retry(agent_id=agent, task_id=task, has_diff=False, tokens=100)

        spin_detector.reset(agent)
    ...
    def test_tokens_accumulated_across_retries(self, spin_detector: SpinDetector) -> None:
        """Tokens should accumulate across retries without a diff."""
        agent = AgentId("agent-acc")
        task = TaskId("task-acc")

        spin_detector.record_retry(agent_id=agent, task_id=task, has_diff=False, tokens=1000)
        result = spin_detector.record_retry(
