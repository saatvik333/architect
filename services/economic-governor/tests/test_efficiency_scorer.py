...
class TestEfficiencyScorer:
    ...
    def test_empty_leaderboard(self, efficiency_scorer: EfficiencyScorer) -> None:
        """Leaderboard should be empty with no recorded tasks."""
        board = efficiency_scorer.compute_scores()
        assert len(board.entries) == 0

    def test_single_agent_score(self, efficiency_scorer: EfficiencyScorer) -> None:
        """A single agent should have rank 1 and normalised score of 1.0."""
        agent = AgentId("agent-solo")
        efficiency_scorer.record_task_completed(
    ...
    def test_multiple_agents_ranked(self, efficiency_scorer: EfficiencyScorer) -> None:
    ...
    def test_failed_tasks_tracked(self, efficiency_scorer: EfficiencyScorer) -> None:
        ...
        efficiency_scorer.record_task_failed(agent_id=agent, tokens=3000, cost_usd=0.003)
    ...
    def test_get_agent_score_existing(self, efficiency_scorer: EfficiencyScorer) -> None:
        ...
        score = efficiency_scorer.get_agent_score(agent)
        assert score.agent_id == agent
        assert score.tasks_completed == 1

    def test_get_agent_score_unknown(self, efficiency_scorer: EfficiencyScorer) -> None:
        """get_agent_score should return a default for unknown agents."""
        score = efficiency_scorer.get_agent_score(AgentId("agent-unknown"))
        assert score.efficiency_score == 0.0
        assert score.tasks_completed == 0

    def test_efficiency_formula(self, efficiency_scorer: EfficiencyScorer) -> None:
    ...
    def test_zero_quality_yields_zero_efficiency(self, efficiency_scorer: EfficiencyScorer) -> None:
